...
class Order {
  ...
  async make() {
    ...
    this.makerArguments.forEach((argument) => {
      if (this.shouldSend(argument)) {
        dataTypes.push(argument.dataType);
        values.push(this.makerValues[argument.name]);
      }
    })
  ...
  async take(taker, takerValues) {
    this.checkDataTypes();
    const makerValuesBytes = this.serializeData(this.makerArguments, this.makerValues);
    const takerValuesBytes = this.serializeData(this.takerArguments, takerValues);

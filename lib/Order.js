...
class Order {
  ...
  async post(poster) {
    if(!poster) poster = this.maker;
    await this.checkArguments();
    this.posterSignature = await Signature.generate(this.web3, this.hashContractData(), poster )
  }
  ...
  recoverMaker() {
    if (this.makerValues.signature === undefined) {
      return this.needsToBeSigned();
    } else {
